...
function sendMail(request, response) {
  ...
  const mailOptions = {
    ...
    to: readConfig('email'),
    subject: 'New PaulaD.me Submission',
    html: `<h2>From</h2>
  ...
  transporter.sendMail(mailOptions, (error, info) => {
    if (error) {
      response.json({ status: { error: 'Uh-oh! Looks like something went wrong 😬' }});
    } else {
      response.json({ status: { success: 'Awesome! I\'ll be in touch soon 😎' }});
    }
  });
}
